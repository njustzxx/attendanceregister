    ...
    public function html_table() {
        ...
        if ( $this->userSessions ) {
            ...
            foreach ($this->userSessions as $session) {
                ...
                //$fullnameStr = fullname($this->thisuser);
                // Offline Delete button (if Session is offline and the current user may delete this user's offline sessions)
                if ( !$session->onlinesess && $this->userCapabilites->canDeleteThisUserOfflineSession($session->userid) && ($session->login ) ) {
 ...
 * $table->head = array(
        ...
        if( $this->users ) {
            ...
            foreach ($this->users as $user) {
                $rowcount++;
                $thisUserSsesions=new attendanceregister_user_sessions($this->register,$rowcount,$this->userCapabilities);
                echo html_writter::table($userSessions->html_table())
                $userAggregate = null;

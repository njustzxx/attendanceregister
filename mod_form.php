...
class mod_attendanceregister_mod_form extends moodleform_mod {
    ...
    function definition() {
        ...
        $refStart = make_timestamp($refDate['year'], $refDate['mon'], $refDate['mday'], $refDate['hours'] );
        $mform = $this->_form;
        ...
        //the internal that student should submit
        
        $submitinterval = array(
            1=> ('1'.$day),
            3=> ('3'.$days),
            7=> ('7'.$days),
            15=> ('15'.$days),
        ...
        $mform->addElement('select', 'submitinterval', get_string('submitinterval', 'attendanceregister'), $submitinterval);
        $mform->addHelpButton('submitinterval', 'submitinterval', 'attendanceregister');
        $mform->setDefault('submitinterval', ATTENDANCEREGISTER_DEFAULT_SUBINTERVAL_CERTIFICABLE_CERTIFICABLE );
        $mform->disabledIf('submitinterval', 'offlinesessions');
        
        //date that the work start
        $mform->addElement('date_time_selector', 'begindate', get_string('begindate', 'attendanceregister'), array( 'defaulttime' => $refStart, 'optional' => false )  );
        $mform->addRule('begindate', get_string('required'), 'required');
        $mform->disabledIf('begindate', 'offlinesessions');
